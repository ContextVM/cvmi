...
export async function runSync(_args: string[], options: SyncOptions = {}): Promise<void> {
  ...
  console.log();
  p.intro(pc.bgCyan(pc.black(' cvmi sync ')));
  ...
  if (options.force) {
    ...
    for (const skill of discoveredSkills) {
      const locked = localLock.skills[skill.name];
      if (locked && locked.source === skill.packageName) {
    ...
    if (upToDate.length > 0) {
      p.log.info(pc.dim(`${upToDate.length} skill(s) already up-to-date (skipped)`));
    }
  }
  ...
  const universalAgents = installedAgents.filter(isUniversalAgent);
  ...
  if (options.agent && options.agent.length > 0) {
    ...
    const otherAgents = installedAgents.filter((a) => !isUniversalAgent(a));

    if (otherAgents.length === 0) {
      // Only universal agents - use all installed
      targetAgents = installedAgents;
    } else {
      const otherChoices: { value: string; label: string; hint: string }[] = otherAgents.map(
        (a) => ({
          value: a,
      ...
      const selected = await p.multiselect({
        ...
        initialValues: installedAgents.filter((a) => !universalAgents.includes(a)),
        required: true,
      });
      ...
      targetAgents = (selected as string[]).filter((a): a is AgentType =>
        (Object.keys(agents) as string[]).includes(a)
      );
      // Always include universal agents
      targetAgents = [...new Set([...targetAgents, ...universalAgents])];
    }
  }
